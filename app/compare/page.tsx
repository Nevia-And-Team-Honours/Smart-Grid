...
const ModelComparison: React.FC = () => {
  ...
  useEffect(() => {
    const fetchModelPerformance = async () => {
      ...
      try {
        setLoading(true);
        const response = await fetch("http://localhost:5000/api/model-performance");
        if (!response.ok) {
  ...
  const handleModelChange = async (value: string) => {
    try {
      ...
      const response = await fetch("http://localhost:5000/api/switch-model", {
        method: "POST",
        headers: {
  ...
  const handleCompareModels = async () => {
    try {
      ...
      const response = await fetch("http://localhost:5000/api/compare-models");
      if (!response.ok) {
